    ...
    <section id="values" className="py-20 bg-background">
      <div ref={elementRef} className="container max-w-7xl mx-auto px-4">
        ...
        <div className="text-center mb-16">
          <h2
            className={`text-4xl md:text-5xl font-bold text-foreground mb-6 transition-all duration-1000 ease-out ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}
            style={{ transitionDelay: isVisible ? "0.2s" : "0s" }}
          >
            Our Core Values:{" "}
            <span className="text-brand-orange">C.L.A.R.I.T.Y.</span>
          </h2>
          <p
            className={`text-lg text-muted-foreground max-w-4xl mx-auto leading-relaxed transition-all duration-1000 ease-out ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}
            style={{ transitionDelay: isVisible ? "0.4s" : "0s" }}
          >
            Every service we provide is guided by our core values, ensuring
            comprehensive support that addresses both digital resilience and
          ...
          {values.slice(0, 4).map((value, index) => {
            ...
            return (
              <div
                ...
                style={{
                  transitionDelay: isVisible ? `${0.6 + index * 0.1}s` : "0s",
                }}
              >
            ...
            {values.slice(4).map((value, index) => {
              ...
              return (
                <div
                  ...
                  className={`relative bg-card rounded-xl shadow-sm p-6 text-center hover:shadow-md transition-all duration-1000 ease-out overflow-hidden ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}
                  style={{
                    transitionDelay: isVisible ? `${1.0 + index * 0.1}s` : "0s",
                  }}
                >
                  {/* Large Background Letter */}
                  <div className="absolute top-2 right-4 text-8xl font-bold text-brand-orange/10 select-none pointer-events-none">
