    ...
    if (isGHLReady) {
      try {
        ...
        if (window.sendChatMessage) {
          ...
          const ghlInput = document.querySelector(
            "[data-widget-id] input, .chat-widget input",
          ) as HTMLInputElement;
          const ghlButton = document.querySelector(
            '[data-widget-id] button, .chat-widget button[type="submit"]',
        ...
        <div className="mb-4 w-[350px] h-[500px] bg-background rounded-2xl shadow-2xl flex flex-col border border-border">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {messages.map((message, index) => (
              ...
              >
                <div
                  className={`max-w-[80%] p-3 rounded-lg ${message.isUser ? "bg-primary text-primary-foreground" : "bg-muted text-foreground"}`}
                >
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                ...
                className="relative rounded-full shadow-lg transition-all hover:scale-105"
              >
                <img
                  src={jesseAvatar}
                  alt="Chat"
